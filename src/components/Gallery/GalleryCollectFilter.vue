  ...
  <div class="search-filter-form __zw-search-filter-form">
    ...
    <div class="search-field">
      <div class="form-item">
        ...
        <div class="cont flex">
          <CheckBox :data="filterData.collectSourceMap" v-model="form.collect_source_type"></CheckBox>
        </div>
      </div>
      <div class="form-item">
        <div class="label">
...
export default {
  ...
  methods: {
    ...
    formateFilterParams() {
      ...
      for(let i in this.designerConfig.collectSourceMap){
        collectSourceMap.push({id: i, name: this.designerConfig.collectSourceMap[i]});
      }
      this.filterData = { ...this.designerConfig, extList, signList, collectSourceMap };
    },
    formHandler(type) {
      switch (type) {
        case "reset":
